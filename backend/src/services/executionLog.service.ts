...
import type { LogExecutionData } from "../types/types";

const logExecution = async (data: LogExecutionData): Promise<void> => {
  try {
...
const cleanupOldLogs = async (daysToKeep: number = 7): Promise<number> => {
  try {
...
export { logExecution, getExecutionStats, getRecentExecutions, cleanupOldLogs };
