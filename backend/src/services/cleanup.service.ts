...
import { cleanupOldLogs } from "./executionLog.service";

const cleanupOldSandboxes = async (
  daysToKeep: number = 7
): Promise<{
...
const cleanupOrphanedSchemas = async (): Promise<number> => {
  try {
    const client = await pool.connect();
    try {
...
const performFullCleanup = async (
  ...
  try {
    ...
    const sandboxCleanup = await cleanupOldSandboxes(daysToKeep);

    const orphanedCleanup = await cleanupOrphanedSchemas();

    const executionLogsDeleted = await cleanupOldLogs(daysToKeep);
